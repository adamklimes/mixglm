...
Function `predict()` can be also used for prediction by specifying argument
`newdata`. For example, we can predict domains for precipitation at the 
end of 21. century:

```{r predict}
predF <- predict(mod, threshold = 0.1, 
...
plot(modPT, form = treeCover ~ tempSt)

# Stability landscape along temperature gradient (for precipSt == 0)
landscapeMixglm(modPT, treeCover ~ tempSt, threshold = 0.1, addPoints = FALSE)

# Stability lanscape along precipitation gradient (for temperature 20°C)
landscapeMixglm(modPT, treeCover ~ precipSt, threshold = 0.1, 
...
max(nStableStates) # Maximum number of stable states is 2
...
along whole predictor gradient. Although some measures of resilience are not
