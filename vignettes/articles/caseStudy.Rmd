...
a precipitation gradient. The used tree cover data is from 2001. The replication 
is done using tree cover for the year 2025 for the whole South America. \
\
...
In this case study, we will be modelling the stability landscape along precipitation
and temperature gradients. There are only a few observations with very high
...
First, let's model the stability landscape along the precipitation gradient. 
Tree cover has an upper (100%) as well as lower (0%) limit, therefore we will 
use not normal but beta mixture. A beta-distributed variable can have values 
from 0 to 1 (excluding 0 and 1). Thus we calculate the proportion of tree cover 
...
curve. Apart from the stability curve, the figure also shows estimates of each 
...
to get stable states and tipping points for all modeled observations 
...
The function `predict()` can be also used for prediction by specifying argument
...
We can still use 2D visualization functions `plot()` and `landscapeMixglm()`.
However, they show us only one gradient with the other predictor fixed to zero
or a specified value in the case of `landscapeMixglm()`. Therefore, fit of 
...
unreliable. Generally, we would recommend avoiding highly correlated predictors 
...
defined for these areas or cases such as distance to the closest tipping point, 
